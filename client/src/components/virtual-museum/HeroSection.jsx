  ...
  return (
    <div className="relative min-h-[80vh] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 text-center text-white">
        {/* Main Heading */}
        <div className="mb-8">
          <div className="inline-flex items-center gap-2 bg-green-600/20 backdrop-blur-sm px-4 py-2 rounded-full border border-green-400/30 mb-6">
            <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
            <span className="text-green-100 text-sm font-medium">Virtual Museum Experience</span>
          </div>
          
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold mb-6 leading-tight">
            <span className="bg-gradient-to-r from-yellow-200 via-yellow-200 to-yellow-100 bg-clip-text text-transparent">
              EthioHeritage
            </span>
            <br />
        ...
        <div className="mb-12">
          <div className="max-w-2xl mx-auto">
            <div className="relative">
                 <Search className="absolute left-4 top-4/5 transform -translate-y-1/2 text-gray-400 w-5 h-5 z-10" />
              <Input
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                onKeyPress={handleKeyPress}
                placeholder="Search artifacts by name, culture, period, or region..."
                className="pl-12 pr-4 py-4 text-lg bg-white/10 backdrop-blur-md border-white/20 text-black placeholder:text-gray-300 focus:border-green-400 focus:ring-green-400/30 rounded-xl"
              />
              <Button
                onClick={handleSearch}
                className="absolute right-0 top-5/5 transform -translate-y-1/2 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg"
              >
                Explore
              </Button>
            </div>
          ...
          {stats.map((stat) => (
            ...
            >
              <div className="flex items-center justify-center mb-3">
                <stat.icon className="w-8 h-8 text-green-400 group-hover:scale-110 transition-transform duration-300" />
              </div>
